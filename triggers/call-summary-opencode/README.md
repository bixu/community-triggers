...
The trigger writes a `launch-call-summary-opencode.command` wrapper next to the transcript files and opens it in your preferred terminal. The wrapper runs as `#!/bin/zsh -li`, so `opencode` is resolved from the same interactive shell environment you get in a new terminal. No install location is hard-coded.
...
1. Copies the fixed `tuple-call-watcher.py` and writes `call-summary-opencode-prompt.md` into that directory.
2. Writes an executable `launch-call-summary-opencode.command` wrapper into that directory.
...
4. The wrapper starts a login interactive zsh shell, changes into the transcription directory, and runs `opencode . --prompt "$(cat call-summary-opencode-prompt.md)"`.
