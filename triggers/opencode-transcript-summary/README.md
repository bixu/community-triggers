...
The trigger writes a `summarize-in-opencode.command` wrapper next to the transcript files and asks macOS to open it. The wrapper runs as `#!/bin/zsh -li`, so `opencode` is resolved from the same interactive shell environment you get in a new terminal. No install location is hard-coded.
