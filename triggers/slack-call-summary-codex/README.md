...
1. Writes `slack-call-summary-codex-prompt.md` into that directory, including the configured recipient, the transcript file paths, and all summarization instructions.
2. Headlessly launches a login zsh (`nohup zsh -lc`, so `codex` resolves from your normal PATH — no terminal window) that runs `codex exec` in the transcription directory with the summary prompt on stdin.
3. Codex reads `events.jsonl` and `transcriptions.jsonl`, composes the summary message, and sends it using its Slack connector tools.
...
- **Codex run fails or hangs**: Check `slack-call-summary-codex.log` in the transcription directory for the run's output.
...
- **Codex not found**: Make sure `codex` is on your login-shell PATH. Open a new terminal and run `codex --version` to confirm. The trigger launches Codex via a login zsh (`zsh -l`) to pick up the same PATH you see in a fresh terminal.
...
To test the trigger without launching Codex, set `SLACK_CALL_SUMMARY_CODEX_DRY_RUN=1`. The script generates the prompt file and exits without starting a background process:
